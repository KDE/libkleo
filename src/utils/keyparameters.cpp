...
void KeyParameters::setControlStatements(const QStringList &controlStatements)
{
    ...
    for (const auto &controlStatement : controlStatements) {
        if (!controlStatement.startsWith(u"%"_s)) {
            qCFatal(LIBKLEO_LOG).nospace() << "Invalid control statement \"" << controlStatement << "\". Control statements start with a %";
        }
    }
#endif
    d->controlStatements = controlStatements;
}
